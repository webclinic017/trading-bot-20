...
def stock_intraday_view(ticker):
    return render_template('stock-intraday.html', intradays=IntradayDAO.read_filter_by_ticker(ticker))


@app.route('/process')
def process_view():
    active_processes = list(map(lambda j: j.name, process_manager.get_processes()))
    inactive_processes = list(filter(lambda j: j not in active_processes, processes.keys()))
...
def process_start_view(process_name):
...
def process_stop_view(process_name):
...
def export_view(data):
    if data == 'intraday':
        ...
        rows_dict = list(map(lambda row: dict(filter(lambda e: not e[0].startswith('_'), row.__dict__.items())), rows))
